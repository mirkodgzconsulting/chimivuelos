...
} from "@/app/actions/manage-clients";
...
export default function ClientsPage() {
  ...
  // Docs Viewer State
  const [docsViewerClient, setDocsViewerClient] =
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const totalCount =
        existingFiles.length + newFiles.length + selected.length;
      if (totalCount > 5) {
        alert(
          `Solo puedes subir un máximo de 5 archivos. Actualmente tienes ${existingFiles.length + newFiles.length} y seleccionaste ${selected.length}.`,
        );
        return;
      }
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "¿Estás seguro de que deseas eliminar este cliente? Se borrarán todos sus datos y archivos.",
      )
    ) {
      return;
    }
  ...
  const handleViewFile = async (file: ClientFile) => {
    try {
      ...
      const storageType =
        file.storage || (file.path.includes("/") ? "r2" : "images");
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog
        open={!!docsViewerClient}
        onOpenChange={(open) => !open && setDocsViewerClient(null)}
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-2 py-4">
            {docsViewerClient?.client_files &&
            docsViewerClient.client_files.length > 0 ? (
              docsViewerClient.client_files.map((doc, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 bg-slate-50 border rounded-md hover:bg-slate-100 transition-colors"
                >
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button
                ...
                className="bg-linear-to-r from-chimipink to-chimicyan hover:opacity-90 transition-opacity text-slate-700 gap-2 font-bold shadow-md border-none"
              >
              ...
              <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    placeholder="maria@chimivuelos.pe"
                  ...
                  {newFiles.length > 0 && (
                    <div className="mt-2 space-y-2">
                      ...
                      {newFiles.map((file, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between p-2 bg-slate-50 rounded-md border border-slate-200 text-sm"
                        >
                  ...
                  {existingFiles.length > 0 && (
                    <div className="mt-2 space-y-2">
                      ...
                      {existingFiles.map((file, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between p-2 bg-blue-50 rounded-md border border-blue-100 text-sm"
                        >
                          <div className="flex flex-col">
                            ...
                            {file.storage && (
                              <span className="text-[9px] text-slate-400 capitalize">
                                {file.storage === "images" ? "Imagen" : "Doc"}
                              </span>
                            )}
                          </div>
                          <Button
                ...
                <DialogFooter>
                  <Button
                    type="submit"
                    className="bg-linear-to-r from-chimipink to-chimicyan hover:opacity-90 transition-opacity text-slate-700 w-full sm:w-auto font-bold shadow-md"
                  >
        ...
        <div className="flex items-center gap-4 p-4 border-b border-slate-100 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="gap-2 text-slate-600 border-slate-200 hover:bg-slate-50 hover:text-slate-900"
            >
                  ...
                  >
                      ...
                      <div className="flex justify-center">
                        ...
                        client.client_files.length > 0 ? (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="text-chimiteal hover:bg-teal-50"
                            onClick={() => setDocsViewerClient(client)}
                          >
                            <FolderOpen className="h-5 w-5" />
                            <span className="ml-1 text-xs">
                              {client.client_files.length}
                            </span>
                          </Button>
                        ) : (
                    ...
                    <td className="px-6 py-4 text-center">
                      <Button
                        size="sm"
                        variant={
                          copiedId === client.id ? "secondary" : "outline"
                        }
            ...
            <div className="flex items-center justify-between px-4 py-4 border-t border-slate-100 bg-slate-50/50 flex-wrap gap-4">
              <div className="text-xs text-slate-500">
                ...
                <span className="font-medium">
                  {Math.min(indexOfFirstItem + 1, clients.length)}
                </span>{" "}
                -{" "}
                <span className="font-medium">
                  {Math.min(indexOfLastItem, clients.length)}
                </span>{" "}
                de <span className="font-medium">{clients.length}</span>{" "}
                resultados
              </div>
