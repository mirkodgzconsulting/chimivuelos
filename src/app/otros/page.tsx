...
import { getOtherServiceByCode } from '@/app/actions/manage-other-services'
...
function OtherServiceTrackingContent() {
    const searchParams = useSearchParams()
    const [code, setCode] = useState('')
    const [result, setResult] = useState<OtherServiceResult | null>(null)
    ...
    const performSearch = async (searchCode: string) => {
        ...
        try {
            const res = await getOtherServiceByCode(searchCode)
            if (res.error) {
                setError(res.error)
            } else if (res.data) {
                setResult(res.data as OtherServiceResult)
            }
        } catch {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            ...
            case 'processing':
                return { label: 'En Proceso', color: 'text-blue-600', bg: 'bg-blue-100', icon: Loader2, desc: 'Servicio en curso.' }
            case 'completed':
            case 'delivered':
                return { label: 'Completado', color: 'text-emerald-600', bg: 'bg-emerald-100', icon: CheckCircle, desc: 'Finalizado exitosamente.' }
            case 'cancelled':
                return { label: 'Cancelado', color: 'text-red-600', bg: 'bg-red-100', icon: XCircle, desc: 'Servicio cancelado.' }
            default:
            ...
            <Card className="border-slate-200 shadow-xl bg-white/80 backdrop-blur-sm">
                <CardHeader className="text-center pb-2">
                    <CardTitle className="text-2xl font-bold text-slate-800">
                        Seguimiento de Otros Servicios
                    </CardTitle>
                    <CardDescription>
                        Ingrese el código de su solicitud para consultar el estado.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <div className="mt-8 space-y-6 animate-in fade-in slide-in-from-top-4">
                            ...
                            <div className="bg-slate-50/80 rounded-xl border border-slate-100 p-5 space-y-5">
                                ...
                                <div className="flex justify-between items-start pb-4 border-b border-slate-200/60">
                                    <div>
                                        <p className="text-[10px] tracking-wider font-bold text-slate-400 uppercase mb-0.5">Código de Solicitud</p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] tracking-wider font-bold text-slate-400 uppercase mb-1">Tipo de Servicio</p>
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-500 font-bold text-xs shrink-0">
                                                <Briefcase className="h-4 w-4" />
                                            </div>
                                            <p className="text-sm font-semibold text-slate-700 truncate capitalize">{result.service_type}</p>
                                        </div>
                                    </div>
                                </div>
