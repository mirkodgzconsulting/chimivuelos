...
export function FlightDocumentRow({ doc }: { doc: FlightDocument }) {
    ...
    const handleAction = async (e?: React.MouseEvent) => {
        e?.stopPropagation() // Prevent bubbling if needed, though this is the main action
    ...
    return (
        <>
            <div 
                ...
                className="cursor-pointer flex items-center justify-between p-3 bg-white/60 border border-white/50 rounded-lg hover:bg-white/80 hover:border-chimipink/30 active:scale-[0.98] active:bg-white/90 transition-all duration-200 group shadow-sm hover:shadow-md"
            >
                ...
                <div className="text-slate-400 group-hover:text-chimipink transition-colors shrink-0">
                ...
                <Dialog open={isOpen} onOpenChange={setIsOpen}>
                    <DialogContent className="w-[95vw] sm:w-full max-w-4xl h-[85vh] sm:h-[80vh] flex flex-col p-0 overflow-hidden rounded-xl">
                        ...
                        <div className="flex-1 bg-slate-100 relative w-full h-full flex items-center justify-center overflow-auto p-4">
                            {loading ? (
                                <Loader2 className="w-8 h-8 text-chimipink animate-spin" />
                            ) : url ? (
                                <>
                                    {isImage && (
                                        <img 
